...
with conn:
  cur = conn.cursor()
  cur.execute('SELECT year, month, day, gisjoin, wrluri_impute, alpha, beta, tau FROM zoning_attributes WHERE urbanized1990=1')
  results = cur.fetchall()
  year = []
  month = []
  day = []
  ...
  for result in results:
    ...
    day.append(result[2])
    gisjoin.append(result[3])
    wrluri.append(result[4])
    alpha.append(result[5])
...
for interval in list(range(1,11)):
  ...
  for gisjoin in gisjoin_set:
    for years in year_pair:
      ...
      if len(first)!=0:
        first = first[['wrluri','alpha','beta','tau']].mean().to_frame().transpose()
        last = muni[muni['year']==years[1]]
        if len(last)!=0:
          last = last[['wrluri','alpha','beta','tau']].mean().to_frame().transpose()
          corr = pd.concat([first,last])
          auto = np.diag(corr.corr())
          if not np.isnan(auto.any()):
            auto_corr = np.vstack([auto_corr, auto])
  auto_corr = auto_corr[~np.isnan(auto_corr).any(axis=1), :]
  means = np.mean(auto_corr, axis=0)
  print(f'Mean {str(interval)}-year lag auto-correlations: {means}, n={auto_corr.shape[0]}')
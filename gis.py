...
import sqlite3
import pymysql
from pymysqlpool.pool import Pool as Pool_sq
import pickle
import fiona
import rtree
import numpy as np
...
# for region in regions:
#     S3FS.get(f'{S3_PATH}gis/gis/PAD/PADUS2_1_Region{region}_Shapefile', f'PADUS2_1_Region{region}_Shapefile', recursive=True)
#     for set in sets:
#         pad = gpd.read_file(f'PADUS2_1_Region{region}_Shapefile/PADUS2_1{set}_Region{region}.shp')
#         none_pad = pad[ ~(pad['Date_Est'].apply(type) == str) ]
#         old_pad = pad[ pad['Date_Est'].apply(type) == str ]
#         old_pad = old_pad[ old_pad['Date_Est'].apply(int) <= 2010 ]
#         pad = pd.concat([old_pad, none_pad])[['geometry']]
#         public_lands = pd.concat([public_lands, pad])
...
S3FS.get(f'{S3_PATH}gis/gis/undev_land_cover_2011.gpkg', 'undev_land_cover_2011.gpkg')
S3FS.get(f'{S3_PATH}gis/gis/undev_slope/undev_slope.gpkg', 'elev.gpkg')

public_lands = gpd.read_file('PAD_Concat.gpkg').to_crs('EPSG:4326')[['geometry']]
water_wet = gpd.read_file('undev_land_cover_2011.gpkg').to_crs('EPSG:4326')[['geometry']]
elev = gpd.read_file('elev.gpkg').to_crs('EPSG:4326')[['geometry']]

undev = pd.concat([water_wet,elev,public_lands])['geometry'].reset_index(drop=True)
undev_sindex = undev.sindex

## Developable Urban Areas

# S3FS.get(f'{S3_PATH}gis/gis/2010_ua', '2010_ua', recursive=True)
uas = gpd.read_file('2010_ua/US_urb_area_2010.shp').to_crs('EPSG:4326')

char = "'"

def clip_worker(row):
    dev_city = uas.iloc[row]

    undevelopable = undev_sindex.query(dev_city['geometry'])
    undevelopable_geom = undev.iloc[undevelopable]
    if len(undevelopable_geom)==0:
        return
    undevelopable_geom = gpd.GeoDataFrame(undevelopable_geom).to_crs('EPSG:4326')

    dev_city = gpd.GeoDataFrame({'GISJOIN':[dev_city['GISJOIN']],'NAME':[dev_city['NAME10']],'UACODE':[dev_city['UACE10']],'geometry':[dev_city['geometry']]}, crs='EPSG:4326')
    
    try:
        dev_city = dev_city.overlay(undevelopable_geom, how='difference', keep_geom_type=True)

        return dev_city

    except:
        for index, row in undevelopable_geom.iterrows():
            row = gpd.GeoDataFrame({'geometry':[row['geometry']]}, crs='EPSG:4326')
            try:
                dev_city = dev_city.overlay(row, how='difference', keep_geom_type=True)
            except:
                pass
        
        return dev_city

tasks = list(range(len(uas)))[433:]
if __name__ == '__main__':
    threads = Pool(processes=2)
    res = list(tqdm(threads.imap_unordered(clip_worker,tasks), total=len(tasks)))

    threads.close()
    threads.join()

    results_df = pd.concat(res)
    results_df.to_file('dev_city2010.gpkg')

    S3FS.put('dev_city2010.gpkg',f'{S3_PATH}gis/gis/dev_city2010.gpkg')

# Obtain Urbanized Block Groups
# S3FS.get(f'{S3_PATH}gis/gis/2010_blckgrp','2010_blkckgrp', recursive=True)

blckgrp = gpd.read_file('2010_blckgrp/US_blck_grp_2010.shp')[['GISJOIN','geometry']]
blckgrp['UACODE'] = np.nan
uas = gpd.read_file('2010_ua/US_urb_area_2010.shp')[['UACE10','geometry']].rename(columns={'UACE10':'UACODE'})
...
# # Find distribution of commutes
urb_blckgrp = gpd.read_file('urb_blckgrp.gpkg')
comm_blckgrp = pd.read_csv(f'{S3_PATH}gis/csv/2010_blckgrp_commutes/nhgis0047_ds176_20105_blck_grp.csv',  encoding='latin-1',low_memory=False)
comm_blckgrp = comm_blckgrp.merge(urb_blckgrp, how='left', on='GISJOIN')
print(comm_blckgrp)
def comm_distribution(x, alpha):
    # alpha is a tuning parameter for logistic function through which `diff` is transformed. Recommended setting ~3
    ...
    for i in list(range(1,13)):
        if len(str(i)) == 1:
            i = f'0{i}'
        pop_dup = pop + x[f'JM2M0{i}']
        prob = x[f'JM2M0{i}'] / pop
        probs.append(prob)
        min_share = 0 
        for min in list(range(1,int(i)+1)):
            if len(str(min)) == 1:
                min = f'0{min}'
            min_share += x[f'JM2M0{min}']/pop_dup
        max_share = 0 
        for max in list(range( int(i), 13)):
            if len(str(max)) == 1:
                max = f'0{max}'
            max_share += x[f'JM2M0{max}']/pop_dup
        diff = max_share - min_share
        diff = (2/(1+math.exp(-alpha*diff)))-1
...
comm_blckgrp['pop'] = comm_blckgrp['JM2M001']+comm_blckgrp['JM2M002']+comm_blckgrp['JM2M003']+comm_blckgrp['JM2M004']+comm_blckgrp['JM2M005']+comm_blckgrp['JM2M006']+comm_blckgrp['JM2M007']+comm_blckgrp['JM2M008']+comm_blckgrp['JM2M009']+comm_blckgrp['JM2M010']+comm_blckgrp['JM2M011']+comm_blckgrp['JM2M012']
...
for index, row in tqdm(comm_blckgrp.iterrows(), total=len(comm_blckgrp)):
    ...
    comm_blckgrp.at[index, "probs"] = [probs]
comm_blckgrp = comm_blckgrp[['GISJOIN','UACODE','values','probs','pop']]
comm_blckgrp.to_csv(f'{S3_PATH}gis/csv/2010_comm_blckgrp_dist.csv', index=False)

# Construct Block Group Mu
comm_blckgrp = pd.read_csv(f'{S3_PATH}gis/csv/2010_comm_blckgrp_dist.csv')
...
comm_blckgrp.to_csv(f'{S3_PATH}gis/csv/2010_blck_grp_mu_comm.csv', index=False)

# Preparatory work For Gini
S3FS.get(f'{S3_PATH}gis/gis/2010_place','2010_place',recursive=True)
S3FS.get(f'{S3_PATH}gis/gis/2010_county','2010_county',recursive=True)
S3FS.get(f'{S3_PATH}gis/gis/2010_ua','2010_ua',recursive=True)
S3FS.get(f'{S3_PATH}gis/gis/dev_city2010.gpkg','dev_city2010.gpkg')

places = gpd.read_file('2010_place/US_place_2010.shp')[['GISJOIN','geometry']].to_crs('EPSG:4326')
places['source'] = 'places'
counties = gpd.read_file('2010_county/US_county_2010.shp')[['GISJOIN','geometry']].to_crs('EPSG:4326')
counties['source'] = 'counties'
uas = gpd.read_file('2010_ua/US_urb_area_2010.shp').to_crs('EPSG:4326').rename(columns={'UACE10':'UACODE'})
juri = pd.concat([counties,places])
...
dev_uas = gpd.read_file('dev_city2010.gpkg').to_crs('EPSG:4326').rename(columns={'UACE10':'UACODE'})
juri = gpd.GeoDataFrame()
for ua in tqdm(ua_set):
    juris = new[new['UACODE']==ua]
    dev = dev_uas[dev_uas['UACODE']==ua]
...
juri = juri[['GISJOIN_1','source','UACODE_1','geometry']].rename(columns={'GISJOIN_1':'GISJOIN','UACODE_1':'UACODE'})
juri.to_file('dev_juris_2010.gpkg')
S3FS.put('dev_juris_2010.gpkg',f'{S3_PATH}gis/gis/dev_juris_2010.gpkg')

# Construct Gini
S3FS.get(f'{S3_PATH}gis/gis/2010_blckgrp','2010_blckgrp',recursive=True)
juris = gpd.read_file('dev_juris_2010.gpkg').to_crs("EPSG:3347")
juris['UACODE'] = juris['UACODE'].astype(float)

blckgrp_geom = gpd.read_file('2010_blckgrp/US_blck_grp_2010.shp')[['GISJOIN','geometry']]
blckgrp = pd.read_csv(f'{S3_PATH}gis/csv/2010_blck_grp_mu_comm.csv')
blckgrp = blckgrp.merge(blckgrp_geom, how='left', on='GISJOIN')
blckgrp = gpd.GeoDataFrame(blckgrp).to_crs("EPSG:3347")
...
new = new.to_crs('EPSG:4326')
new.to_file('2010_gini_base.gpkg')

gini = gpd.read_file('2010_gini_base.gpkg')
gini['mult'] = gini['mu_comm']*gini['area']
...
gini = places.merge(city, how='left', on='UACODE')
gini['frac'] = gini['mult']/gini['city_sum']
gini.to_csv(f'{S3_PATH}gis/csv/2010_juris_weights.csv', index=False)
gini = gini[['UACODE','frac']]
...
final.to_csv(f'{S3_PATH}gis/csv/2010_gini.csv', index=False)